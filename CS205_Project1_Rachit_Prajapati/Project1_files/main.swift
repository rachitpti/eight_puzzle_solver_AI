...
struct PriorityQueue<Element> { //generic type Element which is used as a type Int in the application
    ...
    } //constructs heeap from the elements array
    ...
    mutating func dequeue() -> Element? {
        ...
        let element = elements.removeLast() //removing the last element from the priorty queue
        if !isEmpty {
            heapifyDown(from: 0) //balancifying the heap
        }
        return element
    }
    ...
    private mutating func heapifyDown(from index: Int) {
        ...
        while true {
            ...
            var optionalSwapIndex: Int? //it only gets the value feeded to it if below if statements satisfy else it remains nil (null)
...
class Problem {
    ...
    private func calculateMisplacedTiles(state: [Int]) -> Int {
        var misplacedTiles = 0 //counts the score of misplaced tile fromm the goal state
    ...
    private func expand(node: Node) -> [Node] {
        ...
        for (loop_row, loop_col) in directions {
            //making new positionn for the empty tile
            ...
            if new_Row >= 0, new_Row < 3, new_Column >= 0, new_Column < 3 {
                ...
                //fetching the heursistic based on the mode inputted by user
                let newHeuristic = calculateHeuristic(state: newState)
              //appending the successive node with the respective heurisitc and classifying the linking of the current node as a parent node
...
//Initialzing the 8 puzzle assignment as a game.
func startGame() {
    print("""
        ...
        //In Swift any data/input we receieve from external source is classified into Optionals (empty value (written as 'nil') and non empty value).
        //To handle the uncertainity of the value fetched, using if-let to get the concrete non empty value
...
//instantiates the solving logic code by 1. capturing the instance representation of the intial/user inputted state and then feeding it to the solving logic.
